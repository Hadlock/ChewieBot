...
$whitelistURL = 'http://reg.davejk.net/tools/list';
$forceKickVerification = '21JZpvySoG6I7XK'; // random string that must be passed in order to bypass lastrun timer
...
foreach ($servers as $server) {
	...
	if ($socket !== False) {
		...
		if (count($nonWhitelistedPlayers) == 0) {
			echo "$server->name: goon-filled (ew).\n";
			socket_close($socket);
			continue;
		}
		...
		if ($FORCEKICK || whitelistKick_init($server->abbv, $cooldownSeconds)) {
			...
			$response = rconCommand($socket, "admin.say 'LOLOCAUST in $secondsBeforeKick seconds. Whitelist or complain at BFGoons.com.' all");
			...
			foreach ($nonWhitelistedPlayers as $player) {
				...
				$reason = $kickReasons[array_rand($kickReasons)];
				$reason = preg_replace("/\'/", "\'", $reason);
				
				$response = rconCommand($socket, "admin.kickPlayer '$player' $reason");
				
				if ($response[0] != 'OK') {
					echo "$server->name: trouble kicking $player: $response[1]\n";
					continue;
				} else
					echo "$server->name: $player kicked: $reason\n";
				
				$totalPlayersKicked++;
				
				rconCommand($socket, "admin.say '$player kicked: $reason' all");
